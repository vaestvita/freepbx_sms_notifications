// Post Call Recording Script: /usr/bin/php /var/lib/asterisk/bin/sms/test.php  ^{ARG3}
// для работы с модулем Web CallBack https://wiki.freepbx.org/display/FPG/Web+CallBack
<?php
...
$filenotify = '/var/lib/asterisk/bin/sms/notify.txt'; // путь до фала с сообщениями уведомлениями оператору
$fileanswered = '/var/lib/asterisk/bin/sms/answered.txt';
...
$query = "SELECT src,dstchannel,disposition FROM cdr WHERE calldate >= '" . $fstepdate .  "' AND calldate <= '" . $fcurdate . "' AND dst = " . $argv[1] . " ORDER BY `calldate` DESC LIMIT 1" . ";"; // формируем запрос
echo $query . "\n";
...
if(preg_match('/^79[0-9]{9}/',$src))	{	//проверяем что номер мобильный
	$row = 1;
	$srcnum = 0;
...
//			echo $data[2] . "-" . $src . "\t" . $data [1] . "-" . $stepdate . "\t" . $data[4] . "\n";
			if (($data[2] == $src) &&			//если номер есть в логе
...
//				echo "СМС не отправлялась";
	...
	if($srcnum) {							// если смс надо отправить
		...
		foreach ($lines as $line_num => $line) {//читаем построчно
			$num = substr($line,0,strpos($line,';'));	//выделем внутреннийй номер
			$str = substr($line,strpos($line,';')+1);	//выделяем тескт сообщения
			$ustr = urlencode($str);			// перобразуем в код
			$ustr = str_replace('%5Cn','%0D%0A',$ustr);	// исправляем переводы строки
			if($num == $argv[1]) {		//если внутренний номер найден
...
if ($dialstatus != "ANSWERED"){
	$stepdate = $curdate - $stepnotify*60;	//вычисляем сдвиг по времени
	$row = 1;
	$srcnum = 0;
	if (($handle = fopen($log, "r")) !== FALSE) { 	// получаем указатель на файл с логом
		while (($data = fgetcsv($handle, 1000, ";")) !== FALSE) {	// построчто читаем фал лога
...
//answered call
if ($dialstatus == "ANSWERED"){
	$lines = file($fileanswered);		//читаем файл с сообщеиями оператору
	foreach ($lines as $line_num => $line) {//читаем построчно
